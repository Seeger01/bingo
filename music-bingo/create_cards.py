...
input_file = './input_tidal.csv'
center_figure = '<img src="EVENTLAGERET-LOGO-E1.png"/>'
...
with open(save_path, 'w') as f:
    ...
    for i in range(numbersheets):
        ...
        f.write("<table>")
        f.write("<tr>")
        bingo = ["<th>{}</th>".format(l) for l in list([id])]
        for th in bingo:
            f.write(th)
        f.write("</tr>")
        ...
        f.write("<table>")
        f.write("<tr>")
        card_ids = ["<th>{}</th>".format(l) for l in list('BINGO')]
        for th in card_ids:
            f.write(th)
        f.write("</tr>")
